...
import { getQuestionsFromBank, isQuestionBankAvailable } from "./questionBank";

// Configure Azure OpenAI (optional - only used if question bank is not available)
...
if (process.env.AZURE_OPENAI_API_KEY && process.env.AZURE_OPENAI_ENDPOINT) {
  ...
  console.log("[OpenAI] Azure OpenAI configured (optional)");
} else {
  console.log("[OpenAI] Azure OpenAI not configured - will use local question bank");
}
...
export async function generateMathQuestions(
  ...
  // Priority 1: Try local question bank (no API needed!)
  if (isQuestionBankAvailable()) {
    try {
      const questions = getQuestionsFromBank(gradeLevel, count);
      console.log(`[QuestionBank] Using ${questions.length} pre-generated questions for grade ${gradeLevel}`);
      return questions;
    } catch (error: any) {
      console.warn("[QuestionBank] Error loading questions:", error.message);
      // Fall through to next option
    }
  }

  // Priority 2: Try Azure OpenAI (if configured)
  if (openai) {
    try {
...
- Ensure difficulty is appropriate for grade ${gradeLevel}`;
      ...
      console.log(`[Azure OpenAI] Generated ${questions.length} questions for grade ${gradeLevel}`);
      return questions;
    } catch (error: any) {
      console.error("[Azure OpenAI] Error generating questions:", error.message);
      // Fall through to fallback
    }
  }

  // Priority 3: Fallback to hardcoded sample questions
  console.log("[Fallback] Using sample questions for grade", gradeLevel);
